...
{
    if (msg == NULL || msg->node_id == NODE_ID_MASTER)
    {
        return;
    }

    Device_t *existing = DeviceManager_FindDevice(msg->node_id);
    if (existing != NULL)
    {
        existing->node_type = msg->node_type;
        existing->online = 1;
        existing->last_heartbeat = HAL_GetTick();
        return;
    }
    ...
    for(int i=0;i<MAX_DEVICE_NUM;i++)
    {
        if(g_device_table[i].online == 0)
        {
            ...
            g_device_table[i].state = NODE_STATE_NORMAL;
