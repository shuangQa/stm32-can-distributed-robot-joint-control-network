...
{
    Heartbeat_Msg_t heartbeatMsg = {0};
...
{
    if (ctrl == 0 || (ctrl->node_id != Local_NodeID &&
                      ctrl->node_id != NODE_ID_BROADCAST))
    {
        return;
    }
...
void App_GetStatus(Status_Msg_t* status)
{
    ...
    *status = (Status_Msg_t){0};
    status->node_id=Local_NodeID;
    status->actual_rpm=g_joint.actual_rpm;
    status->pwm=(uint8_t)g_pwm;
    status->state=(g_state == NODE_FAULT) ? NODE_STATE_FAULT : NODE_STATE_NORMAL;
